...
namespace
{
    ...
    HANDLE safe_handle(HANDLE h) noexcept { return (h == INVALID_HANDLE_VALUE) ? nullptr : h; }

#if defined(_DEBUG) || defined(PROFILE)
    template <UINT TNameLength>
    void SetDebugObjectName(_In_ ID3D11DeviceChild* resource, _In_ const char (&name)[TNameLength]) noexcept
    {
...
#undef ISBITMASK
    ...
    //--------------------------------------------------------------------------------------
    DXGI_FORMAT MakeLinear(_In_ DXGI_FORMAT format) noexcept
    {
        switch (format)
        {
        case DXGI_FORMAT_R8G8B8A8_UNORM_SRGB:
                ...
                if (SUCCEEDED(hr) && tex)
                {
                    if (textureView)
                    {
                        ...
                        if (arraySize > 1)
                        {
                            ...
                            SRVDesc.Texture1DArray.MipLevels = (!mipCount) ? static_cast<UINT>(-1) : desc.MipLevels;
                            SRVDesc.Texture1DArray.ArraySize = static_cast<UINT>(arraySize);
                        }
                        else
                        {
                            SRVDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE1D;
                            SRVDesc.Texture1D.MipLevels = (!mipCount) ? static_cast<UINT>(-1) : desc.MipLevels;
                        }
                    ...
                    if (textureView)
                    {
                        ...
                        if (isCubeMap)
                        {
                            ...
                            else
                            {
                                ...
                                SRVDesc.TextureCube.MipLevels = (!mipCount) ? static_cast<UINT>(-1) : desc.MipLevels;
                            }
                        }
                        else if (arraySize > 1)
                        {
                            SRVDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2DARRAY;
                            SRVDesc.Texture2DArray.MipLevels = (!mipCount) ? static_cast<UINT>(-1) : desc.MipLevels;
                            SRVDesc.Texture2DArray.ArraySize = static_cast<UINT>(arraySize);
                        }
                        else
                        {
                            SRVDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
                            SRVDesc.Texture2D.MipLevels = (!mipCount) ? static_cast<UINT>(-1) : desc.MipLevels;
                        }
        ...
        case D3D11_RESOURCE_DIMENSION_TEXTURE3D:
            {
                ...
                if (SUCCEEDED(hr) && tex)
                {
                    if (textureView)
                    {
                        ...
                        SRVDesc.Texture3D.MipLevels = (!mipCount) ? static_cast<UINT>(-1) : desc.MipLevels;
        ...
        _Outptr_opt_ ID3D11ShaderResourceView** textureView) noexcept
    {
        ...
        {
            auto d3d10ext = reinterpret_cast<const DDS_HEADER_DXT10*>(reinterpret_cast<const uint8_t*>(header) + sizeof(
                DDS_HEADER));
